...
from new_DGP import new_DGP_1, new_DGP_2, new_DGP_3
import json
import numpy as np
...
for scenario in scenario_list:
    for n in n_list:
        for scale in scale_list:
            print("--------------------------------")
            # print(f"n={n}, scale={scale}, max_features={max_features}")
            print(f"n={n}, scale={scale}, scale={scale}")
            ...
            for i in range(N):
                print(f"第{i+1}次实验")
                if scenario == 'DGP1':
                    X, y, y_true, error = new_DGP_1(n=n,p=p,scale=scale)
                elif scenario == 'DGP2':
                    X, y, y_true, error = new_DGP_2(n=n,p=p,scale=scale)
                elif scenario == 'DGP3':
                    X, y = new_DGP_3(n=n,p=p,scale=scale)
                else:
            ...
            # 保存为 JSON 文件
            file_name = f"simulation_results8/rmse_{scenario}_n{n}_scale_{scale}.json"
